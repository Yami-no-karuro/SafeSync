...
def init(dest_path: str):
    db_path: str = os.path.join(dest_path, ".safesync", "data", "db", "safesync-core.db")

    init_root(dest_path)
    init_metadata(dest_path)
    init_db(db_path)
    source_dir(dest_path, db_path)

def init_root(dest_path: str):
    root_path: str = os.path.join(dest_path, ".safesync")

    db_dir_path: str = os.path.join(root_path, 'data', 'db')
    objects_dir_path: str = os.path.join(root_path, 'data', 'objects')
    os.makedirs(db_dir_path, exist_ok = True)
    os.makedirs(objects_dir_path, exist_ok = True)

def init_metadata(dest_path: str):
    root_path : str = os.path.join(dest_path, ".safesync")
    metadata_path: str = os.path.join(root_path, 'metadata.json')
    data: dict = {
